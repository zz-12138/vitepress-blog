...
## 原生开发和vue的区别
1. 原生开发属于命令式编程，关注的是每一步执行步骤，how to do
2. vue开发属于声明式编程，将数据和方法提前声明好，what to do
3. 常用编程范式种类：命令式、声明式、函数式、面向对象...
   
## MVVM、MVC模型（软件体系）：
1. MVC：Model-View-Controller的简称，是一种非常流行的框架架构模式
2. MVVM：Model-View-Viewmodel的简称，vue的框架架构模式
3. ![image](../../public/MVVM模型.png)
...
1. 绑定单个：`:[attr]="value"`
2. 绑定多个：`v-bind="{name: 'wall', src: 'http://xx.com'}"`

## v-on的用法

1. 可以绑定函数、表达式、对象(绑定多个事件)
...
3. ![image](../../public/v-on.png)

## v-show不能和template结合使用

## Vue将被监听的数组的变更方法进行了包裹，当这些方法引起数组改变会触发响应式

1. `push、pop、shift、unshift、splice、sort、reverse`

## v-for中key的作用

1. VNode：虚拟节点，本质是一个JavaScript对象，vue中组件和元素都会转化为一个个vnode
2. template——>vnode(vdom)——>dom
3. key属性主要用在vue的虚拟dom算法，在新旧nodes对比时辨识VNodes
    * 如果不使用key，vue会使用一种**最大限度减少动态元素**并且尽可能尝试就地**修改/复用相同类型元素**的算法
    * 使用key，会基于key的变化**重新排列元素顺序**，并且移除销毁key不存在的元素
    * key必须是唯一且不变的
   
## 计算属性会基于他们的依赖关系进行缓存，只有当数据发生变化时，才会重新计算
...
2. v-model的修饰符：
    ...
    * lazy：默认情况下，v-model在进行双向绑定时，绑定的是input事件，每次输入都会同步；如果加上lazy修饰符那么会将绑定的事件切换为change事件，
    只有在提交时（回车）才会触发
...
## 内置组件：keep-alive，用于组件切换时继续保持组件状态，而不是销毁
...
2. 异步组件和suspense：suspense是一个内置的全局组件，该组件有两个插槽
...
## $parent和$root：在组件中获取到父组件和根组件

## 组件生命周期

1. 每个组件都会经历创建、挂载、更新、卸载的过程
2. 有时候需要在某个阶段添加一些代码逻辑（比如组件创建完成之后请求服务器数据），这个时候就需要生命周期钩子函数
3. ![image](../../public/vue生命周期.png)
4. 缓存组件keep-alive的生命周期
    * 对于缓存组件来说，再次进入、退出组件是不会执行created和unmouted等生命周期函数
    * 此时想监听到组件的创建和卸载就需要activated和deactivated这两个钩子函数

## 组件的v-model（可绑定多个v-model）
...
2. 动态组件
3. 组件根节点
4. 实现原理：当插入或删除包含在transition组件中的元素时
...
## gasp动画库学习：通过JavaScript为css、svg、canvas等设置动画
...
## Mixin：对于组件之间相同代码逻辑，可以使用mixin进行抽取

1. 如果mixin选项和组件对象中选项发生了冲突
    * data函数：保留组件自身数据
...
1. 当实现某一个功能时，**对应的代码逻辑会被拆分到各个属性中**
2. 当组件变得更大、更复杂时，逻辑关注点的列表就会变得很长，**同一个功能的逻辑就会拆分的很分散**
3. 这是难以理解且难以阅读的